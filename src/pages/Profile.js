...
export function Profile() {
  const { user, userInformations, handleSignOut, toggleFavorite } =
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.contentContainer}>
        {!loading &&
          userInformations.favorites.length > 0 &&
          userInformations.favorites.map((favorite, index) => {
            const favoriteData = hoteis.find(
              (hotel) => hotel.nome_hot === favorite
            );
